...
def resize_nearest_neighbor(op, context):
    ...
    assert (output_spatial_sizes[0] % shape[1] == 0), \
        ('Resize Nearest Neighbour: height upsampling factor must be an integer. '
         'Input height = {}, output height = {}, ratio = {}'.format(shape[1], output_spatial_sizes[0],
                                                                    output_spatial_sizes[0] / shape[1]))
    assert (output_spatial_sizes[1] % shape[2] == 0), \
        ('Resize Nearest Neighbour: width upsampling factor must be an integer. '
         'Input width = {}, output width = {}, ratio = {}'.format(shape[2], output_spatial_sizes[1],
                                                                  output_spatial_sizes[1] / shape[2]))
...
def resize_bilinear(op, context):
    ...
    if mode == 'UPSAMPLE_MODE' and (output_spatial_sizes[0] % shape[1] == 0) and (
            output_spatial_sizes[1] % shape[2] == 0):
        upsample_factor_height = output_spatial_sizes[0] // shape[1]
        upsample_factor_width = output_spatial_sizes[1] // shape[2]
...
def crop_and_resize(op, context):
    ...
    try:
        ...
        output_spatial_sizes = context.session.run(op.inputs[3].name, feed_dict=context.input_feed_dict)
    if boxes_name in context.consts and box_ind_name in context.consts:
        ...
        boxes = context.consts[boxes_name]  # (N,4)
        context.builder.add_load_constant(boxes_name + '0', boxes_name + '0', boxes.flatten(),
        ...
        box_ind = context.consts[box_ind_name]  # (N)
        context.builder.add_load_constant(box_ind_name + '0', box_ind_name + '0', box_ind.flatten(),
                                          [box_ind.shape[0], 1, 1])
        context.builder.add_permute(box_ind_name + '1', [1, 0, 2, 3], box_ind_name + '0', box_ind_name + '1')
        context.builder.add_elementwise(boxes_name + '2', [box_ind_name + '1', boxes_name], boxes_name + '_concat',
                                        'CONCAT')
        boxes_name += '_concat'
    else:
...
def argmax(op, context):
    ...
    assert (isinstance(axis_tf, int) or isinstance(axis_tf, np.int32) or isinstance(axis_tf, np.int)), (
...
def strided_slice(op, context):
    ...
    # check for slice along the height and width axis
        ...
        if begin[2] != 0:
            ...
            len(begin) == 4 and len(strides) == 4 and len(end) == 4 and \
            (begin_mask == 7 or (begin[0] == 0 and begin[1] == 0 and begin[2] == 0)):
        ...
        context.builder.add_slice(
            output_name, input_name, output_name,
            'channel', int(begin[-1]), int(end[-1]), int(strides[-1]))
    else:
        assert False, ('Strided Slice case not handled. Input shape = {}, output shape = {}'.format(str(input_shape),
                                                                                                    str(output_shape)))
    context.translated[output_name] = True
...
def slice(op, context):
    ...
    # check for slice along the height and width axis
            ...
            and begin[0] == 0 and begin[1] == 0 and begin[2] == 0 and begin[3] == 0:
        if begin[1] != 0 and begin[2] != 0:
            tmp_output_name = output_name + '_height_sliced'
            tmp_input_name = tmp_output_name
        else:
