...
def _add_split(op, context):
    ...
    if not ((len(input_shape) == 4 and len(common_out_shape) == 4 and common_out_shape[:3] == input_shape[:3]) \
        ...
        raise ValueError('Split op case not handled. Input shape = {}, output shape = {}'.format(str(input_shape),
                                                                                                 str(common_out_shape)))
...
def _add_reduce(op, context, mode):
    ...
    # if len(input_shape) == 4 and len(output_shape) == 3 and axis_ind == 3 and input_shape == [*output_shape, 1]:
    #     context.builder.add_reshape(output_name, input_name, output_name, output_shape, 0)
    #     return
    ...
    if status:
        ...
        if len(input_shape) == len(axis_ind):
            ...
            if len(axis_ind) == 1 and axis_ind[
                0] == 0: axis = 'W'  # TODO - Handle it more robustly. Only works for stylenet. (W,C)--->(1,C)
