...
def crop_image_from_xy(image, crop_location, crop_size, scale=1.0):
    ...
    with tf.name_scope('crop_image_from_xy'):
        ...
        top_left_point = crop_location - crop_size_scaled // 2
        bottom_right_point = top_left_point + crop_size_scaled
        divider = np.array([s[1], s[2]])
        top_left_point /= divider
        bottom_right_point /= divider

        boxes = tf.concat([top_left_point, bottom_right_point], 1, name="debug")

        crop_size = tf.cast(tf.stack([crop_size, crop_size]), tf.int32)
        boxes_idx = tf.range(s[0])
        image_c = tf.image.crop_and_resize(tf.cast(image, tf.float32), boxes, boxes_idx, crop_size, name='crop')
        return image_c
...
def find_max_location(scoremap):
    ...
    with tf.variable_scope('find_max_location'):
        ...
        xy_loc = tf.stack(xy_loc, 0, name="xy_loc")
        return xy_loc
...
def single_obj_scoremap(scoremap):
    ...
    with tf.variable_scope('single_obj_scoremap'):
        ...
        scoremap_softmax = tf.nn.softmax(scoremap, name="scoremap_softmax")  #B, H, W, C --> normalizes across last dimension
        scoremap_fg = tf.reduce_max(scoremap_softmax[:, :, :, 1:], 3, keepdims=True, name="scoremap_fg") # B, H, W
        detmap_fg = tf.round(scoremap_fg, name="detmap_fg") # B, H, W
        ...
        for i in range(s[0]):
            ...
            for j in range(num_passes):
                ...
                objectmap = tf.round(tf.multiply(detmap_fg[i, :, :, 0], objectmap_dil))
...
def detect_keypoints(scoremaps):
    """ Performs detection per scoremap for the hands augmented_keypoints. """
...
def detect_keypoints_batch(scoremaps):
    """ Performs detection per scoremap for the hands augmented_keypoints. """
    s = scoremaps.shape
    assert len(s) == 4, "This function was only designed for 4D Scoremaps."

    keypoint_coords = np.zeros((s[0], s[3], 2))
    for batch in range(s[0]):
        for scoremap in range(s[3]):
            v, u = np.unravel_index(np.argmax(scoremaps[batch, :, :, scoremap]), (s[1], s[2]))
            keypoint_coords[batch, scoremap, 0] = v
            keypoint_coords[batch, scoremap, 1] = u
    return keypoint_coords
...
class EvalUtil:
    ...
    def get_measures(self, val_min, val_max, steps):
        ...
        pck_curve_all = np.mean(np.array(pck_curve_all), 0)  # mean only over augmented_keypoints
